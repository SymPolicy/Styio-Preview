...
- 变更 benchmark 覆盖面或执行路线时，只更新这里和 [`COVERAGE-MATRIX.md`](<styio-workspace>/benchmark/COVERAGE-MATRIX.md:1)
...
覆盖设计与补强方向见 [COVERAGE-MATRIX.md](<styio-workspace>/benchmark/COVERAGE-MATRIX.md:1)。
...
[`benchmark/perf-route.sh`](<styio-workspace>/benchmark/perf-route.sh:1) 默认会一起执行；如果不传 `--execute-iters`，脚本会根据 `--phase-iters` 自动推导一个较小的执行迭代数。
...
[`benchmark/perf-route.sh`](<styio-workspace>/benchmark/perf-route.sh:1) 默认也会跑这一组；如果不传 `--error-iters`，脚本会根据 `--phase-iters` 自动推导一个较小但稳定的失败路径迭代数。
...
[`benchmark/perf-route.sh`](<styio-workspace>/benchmark/perf-route.sh:1) 默认也会跑这一组；如果不传 `--micro-iters`，它默认继承 `--phase-iters`。
...
`soak_deep` 档位由 [`benchmark/CMakeLists.txt`](<styio-workspace>/benchmark/CMakeLists.txt:1) 注入放大量参数：
