...
**Purpose:** 冻结 `Vityo` 对 `pafio` 项目图 published payload 的 handoff 要求。
...
`Vityo` 需要 `pafio project-graph --manifest-path <path> --json [--styio-bin <path>]` 至少提供这些 published keys：
...
4. 如果 `pafio` 已广告 `project_graph v1` 但 `project-graph --json` 退出失败、返回非法 JSON 或 payload 解析失败，`Vityo` 必须把它显示为 published contract failure，并把 adapter capability 降到 `partial`；不能静默表现得像合同正常。
