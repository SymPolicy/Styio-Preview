...
1. `pafio machine-info --json`
2. `pafio project-graph --json`
3. `pafio project-graph --manifest-path <path> --json`
...
4. 如果 `pafio` 已广告 `toolchain_state v1` 但 `tool status --json` 退出失败、返回非法 JSON 或 payload 解析失败，`Vityo` 必须把它显示为 published contract failure，并把 project/toolchain 能力降到 `partial`；不能静默回退得像状态合同正常。
