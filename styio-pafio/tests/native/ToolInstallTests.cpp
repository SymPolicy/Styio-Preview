...
TEST(ToolInstallTests, TopLevelInstallDownloadsPrebuiltStyioAndReportsStatus)
{
  ...
  const ScopedEnvVar pafio_home("PAFIO_HOME", (root / ".pafio-home").string());
  const std::string platform = pafio::DetectToolReleasePlatform();
  const std::string release_target = pafio::DetectStyioClientReleaseTarget(platform);
  ...
  WriteFile(release_dir / "styio.sha256", pafio::Sha256File(published_styio) + "\n");
  ...
  const fs::path managed_binary = payload.at("managed_binary_path").get<std::string>();
  const pafio::ProcessResult version_result = pafio::RunProcess<pafio::ToolError>({
  ...
  EXPECT_EQ(pafio::TrimTrailingNewline(version_result.stdout_text), "styio " + version);

  testing::internal::CaptureStdout();
  const int status_exit_code = pafio::RunCli({
      "--json",
      "tool",
      "status",
  });
  ...
  EXPECT_EQ(status_exit_code, pafio::kExitSuccess);
