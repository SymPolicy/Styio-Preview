...
die() {
  echo "pafio-installer-adapter-smoke: $*" >&2
  exit 1
}

script_dir=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
repo_root=$(cd "$script_dir/../.." && pwd)
installer="$repo_root/scripts/install-pafio.sh"

work_dir=$(mktemp -d "${TMPDIR:-/tmp}/pafio-installer-adapter.XXXXXX")
cleanup() {
  rm -rf "$work_dir"
}
trap cleanup EXIT
...
run_adapter() {
  ...
  PAFIO_INSTALL_OS_RELEASE_FILE="$os_release_file" \
    PAFIO_INSTALL_UNAME_S=Linux \
    PAFIO_INSTALL_UNAME_M="$arch" \
    PAFIO_INSTALL_LIBC="$libc" \
...
echo "pafio installer adapter smoke passed"
