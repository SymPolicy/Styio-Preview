#include "PafioTool/PrebuiltInstall.hpp"
...
std::string NormalizeReleaseRoot(std::string root)
{
  ...
  if (root.empty())
  {
    throw pafio::ToolError("tool release root is empty");
  }
  ...
  throw pafio::ToolError("tool release root must be http://, https://, file://, or an absolute path: " + root);
}
...
bool LinuxUsesMusl()
{
  if (const std::optional<std::string> libc = NonEmptyEnv("PAFIO_TOOL_RELEASE_LIBC"); libc.has_value())
  {
    ...
    throw pafio::ToolError("unsupported PAFIO_TOOL_RELEASE_LIBC value: " + *libc);
  }
...
{
  const fs::path config_path = pafio::ResolvePafioHome() / "config" / "tool-release-root";
...
std::string FetchTextFirstField(const std::string &url)
{
  ...
  {
    throw pafio::ToolError("failed to fetch tool release object '" + url + "': " + pafio::DescribeProcessFailure(result));
  }
  std::string field = FirstField(result.stdout_text);
  if (field.empty())
  {
    throw pafio::ToolError("tool release object is empty: " + url);
  }
  return field;
}
...
void FetchUrlToFile(const std::string &url, const fs::path &path)
{
  ...
  if (size_error || size > kMaxToolBinaryBytes)
  {
    std::error_code ignored;
    fs::remove(temp_path, ignored);
    throw pafio::ToolError("tool release object exceeded response limit: " + url);
  }
  ...
  if (rename_error)
  {
    std::error_code ignored;
    fs::remove(temp_path, ignored);
    throw pafio::CacheError("failed to finalize downloaded tool release: " + path.string());
  }
}
...
{
  return pafio_home / "cache" / "tool-releases" / release_target / platform / version / "styio";
}
...
void MarkExecutable(const fs::path &path)
{
  ...
  if (ec)
  {
    throw pafio::CacheError("failed to mark downloaded styio executable: " + path.string());
  }
}
    ...
    const std::optional<std::string> &explicit_release_root)
{
  ...
  if (const std::optional<std::string> root = NonEmptyEnv("PAFIO_STYIO_RELEASE_ROOT"); root.has_value())
  {
    return ResolvedToolReleaseRoot{
        .root = NormalizeReleaseRoot(*root),
        .source = "PAFIO_STYIO_RELEASE_ROOT",
        .explicit_root = false,
    };
  }
  if (const std::optional<std::string> root = NonEmptyEnv("PAFIO_TOOL_RELEASE_ROOT"); root.has_value())
  {
    return ResolvedToolReleaseRoot{
        .root = NormalizeReleaseRoot(*root),
        .source = "PAFIO_TOOL_RELEASE_ROOT",
        .explicit_root = false,
    };
  }
  if (const std::optional<std::string> root = ReadConfiguredReleaseRoot(); root.has_value())
  {
    return ResolvedToolReleaseRoot{
        .root = NormalizeReleaseRoot(*root),
        .source = "PAFIO_HOME/config/tool-release-root",
        .explicit_root = false,
    };
  }
  return std::nullopt;
...
PrebuiltStyioInstallResult InstallPrebuiltStyio(const PrebuiltStyioInstallRequest &request)
{
  ...
  const fs::path downloaded_binary_path = ToolReleaseDownloadPath(pafio_home, resolved_release_target, platform, version);
