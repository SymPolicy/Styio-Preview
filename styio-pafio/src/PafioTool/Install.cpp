...
{
  return pafio::CanonicalAbsolutePath(root / "bin" / "styio-real");
}
    ...
    const fs::path &current_root)
{
  ...
  const fs::path source_wrapper_path = pafio::ManagedStyioBinaryPath(source_root);
    ...
    std::string_view command_name)
{
  ...
  {
    throw pafio::ToolError(std::string(command_name) + " requires a non-empty --version <compiler-version>");
  }
  if (compiler_channel.has_value() && compiler_channel->empty())
  {
    throw pafio::ToolError(std::string(command_name) + " --channel must be a non-empty string");
  }
...
{
  return SelectInstalledManagedCompiler(pafio_home, request.compiler_version, request.compiler_channel, "tool use");
}
