...
int HandleInstall(const std::vector<std::string> &args, bool as_json)
{
  ...
  try
  {
    ...
    if (can_attempt_prebuilt)
    {
      ...
      {
        throw ToolError(
            "prebuilt styio install requires --release-root, PAFIO_STYIO_RELEASE_ROOT, PAFIO_TOOL_RELEASE_ROOT, or PAFIO_HOME/config/tool-release-root");
      }
    }
    else if (prebuilt_only)
    {
...
int HandleToolUpdate(const std::vector<std::string> &args, bool as_json)
{
  if (args.size() == 1 && args.front() == "--help")
  {
    std::cout << "usage: pafio tool update [styio[@latest]] [--release-root <url>] [--channel <stable|nightly>]\n";
    return kExitSuccess;
  }
  ...
  try
  {
    ...
    {
      throw ToolError(
          "tool update requires --release-root, PAFIO_STYIO_RELEASE_ROOT, PAFIO_TOOL_RELEASE_ROOT, or PAFIO_HOME/config/tool-release-root");
    }
...
int HandleToolUninstall(const std::vector<std::string> &args, bool as_json)
{
  if (args.size() == 1 && args.front() == "--help")
  {
    std::cout << "usage: pafio tool uninstall --version <compiler-version> [--channel <channel>]\n";
    return kExitSuccess;
  }
