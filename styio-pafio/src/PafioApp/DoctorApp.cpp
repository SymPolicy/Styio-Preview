...
void PrintHumanDoctor(const json &payload)
{
  ...
  std::cout << "PAFIO_HOME: " << payload.at("pafio_home").get<std::string>() << '\n';
...
int HandleDoctor(const std::vector<std::string> &args, bool as_json)
{
  ...
  AddProgramCheck(
      ...
      "curl is missing; prebuilt pafio/styio downloads will fail",
      "error");
  AddProgramCheck(
      checks,
      "install_command",
      "install",
      "install is available for install-pafio.sh",
  ...
  if (FindProgramOnPath("sha256sum").has_value() || FindProgramOnPath("shasum").has_value())
  {
    checks.push_back({
        ...
        .status = "ok",
        .message = "sha256sum or shasum is available for install-pafio.sh checksum verification",
    });
  }
  else
  {
    checks.push_back({
        .name = "installer_checksum_tool",
        .status = "warning",
        .message = "sha256sum or shasum is missing; install-pafio.sh cannot verify downloaded pafio checksums",
    });
  }
  AddProgramCheck(
      checks,
  ...
  try
  {
    ...
    else
    {
      checks.push_back({
          .name = "release_root",
          .status = "warning",
          .message = "tool release root is not configured; pafio install styio@latest will use source-build unless --release-root is provided",
      });
    }
  }
  catch (const ToolError &err)
  ...
  try
  {
    ...
    else
    {
      checks.push_back({
          .name = "managed_current_styio",
          .status = "warning",
          .message = "managed current styio is not installed; run pafio install styio@latest",
      });
    }
  }
  ...
  else
  {
    checks.push_back({
        .name = "styio_path_shim",
        .status = "warning",
        .message = "styio command is not on PATH; install-pafio.sh normally installs a shim beside pafio",
    });
  }
  ...
  json payload = {
      {"command", "doctor"},
      {"ok", !HasError(checks)},
      {"pafio_home", pafio_home.empty() ? "" : pafio_home.string()},
