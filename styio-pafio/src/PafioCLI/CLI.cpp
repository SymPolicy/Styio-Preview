...
int HandleToolCommand(const std::vector<std::string> &args, bool as_json)
{
  ...
  if (args.empty())
  {
    return pafio::EmitError(
        {"UsageError", pafio::kExitUsage, "tool requires the 'install', 'list', 'status', 'update', 'uninstall', 'use', or 'pin' subcommand", "tool"},
        as_json);
  }
  ...
  switch (*parsed_subcommand)
  {
    ...
    case ToolSubcommand::List:
      return pafio::HandleToolList(tail, as_json);
    ...
    case ToolSubcommand::Update:
      return pafio::HandleToolUpdate(tail, as_json);
    case ToolSubcommand::Uninstall:
      return pafio::HandleToolUninstall(tail, as_json);
