...
**Purpose:** Separate local package-manager registry client and offline package rules from server-side package distribution and registry control-plane ownership now moved to `pafio`.
...
- deployment and operational baseline handoff to `pafio`
...
- registry HTTP control-plane contract is retained here as a client compatibility reference and is owned operationally by `pafio`
...
- executable server validation steps move to `pafio`; this repo keeps package-manager fetch/publish client gates
...
| Capability | `styio-pafio` coverage |
|------------|-----------------------|
