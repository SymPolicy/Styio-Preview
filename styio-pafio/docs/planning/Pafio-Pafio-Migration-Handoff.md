...
**Purpose:** Define the local package-manager-to-global-platform boundary after moving server, package-distribution, and compile-platform ownership into `pafio`.

**Last updated:** 2026-04-24

## Ownership Boundary

`styio-pafio` owns local package-manager behavior: manifests, lockfiles, resolver
...
`pafio` owns global service behavior: hosted workspaces,
...
compatibility shims while `pafio` gets its own CI and release path.
New service behavior should be implemented in `pafio` first and then
consumed by `styio-pafio` through contracts or released tooling.

`styio-pafio` must keep an offline path. If the required package graph is
available through local sources, vendor output, cache entries, or imported
offline bundles, the package manager should not require `pafio`.

## Docs Rule

When platform service docs change, update `pafio` as the owner. This
