...
This spec applies to agent and maintainer work on `styio-pafio` branches. It covers local pre-commit verification, post-push GitHub Actions monitoring, and failure recovery for repository-local and cross-repository gates.
...
```bash
gh api repos/Unka-Malloc/styio-pafio/rules/branches/ai-dev
```
