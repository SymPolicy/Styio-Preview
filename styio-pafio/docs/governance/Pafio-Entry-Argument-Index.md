...
- `pafio doctor [--json] [--manifest-path <path>] [--release-root <url>] [--channel <stable|nightly>]`
...
- `pafio install styio[@latest] [--release-root <url>] [--source|--prebuilt-only] [--source-root <path>] [--source-rev <ref>] [--channel <stable|nightly>] [--build <minimal>] [--yes|--no-fetch|--offline|--non-interactive]`
...
- reports whether the release root came from `--release-root`, `PAFIO_STYIO_RELEASE_ROOT`, `PAFIO_TOOL_RELEASE_ROOT`, or `PAFIO_HOME/config/tool-release-root`
- checks `curl`, `install`, `sha256sum`/`shasum`, `git`, `cmake`, and the `styio` PATH shim
- reports managed compiler state using the same payload shape as `pafio tool status --json`
...
- in `build` mode, the default official source origin is `https://github.com/SymPolicy/Styio-Preview.git`
...
- `--release-root <url>`
  ...
  - falls back to `PAFIO_STYIO_RELEASE_ROOT`, `PAFIO_TOOL_RELEASE_ROOT`, then `PAFIO_HOME/config/tool-release-root`
- `--source`
  - optional
...
- attempts prebuilt install first when a release root is passed, exported, or persisted by `install-pafio.sh`
...
- source fallback fetches source from `PAFIO_STYIO_SOURCE_ORIGIN` when set, otherwise from `https://github.com/SymPolicy/Styio-Preview.git`
- source fallback uses `PAFIO_STYIO_SOURCE_REF` when set; otherwise `latest` maps to `main`
...
```text
curl -fsSL <release-root>/tools/pafio/install-pafio.sh | sh -s -- --base-url <release-root> [--install-dir <dir>]
```
...
- `--base-url <url>`
  ...
  - platform release root containing `tools/pafio/channel/<channel>/<platform>/version`
  - falls back to a directory containing a direct `pafio` binary when the default channel pointer is absent
- `--channel <name>`
  - optional
  - release channel to resolve under `tools/pafio/channel/`
  - defaults to `latest`
- `--version <value>`
  - optional
...
- `--no-release-root-config`
  - optional
  - skips writing `PAFIO_HOME/config/tool-release-root`
- `--print-platform`
  - optional
...
- resolves the selected channel through `tools/pafio/channel/<channel>/<platform>/version`
- downloads `tools/pafio/releases/<version>/<platform>/pafio` and verifies `pafio.sha256`
...
- writes `PAFIO_HOME/config/tool-release-root` when installed from a platform release root so later `pafio install styio@latest` can reuse the same root
