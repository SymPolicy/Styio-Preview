...
pafio doctor
pafio doctor --json
pafio doctor --manifest-path path/to/pafio.toml --release-root https://packages.styio.dev
```

`pafio doctor` reports local prerequisites, `PAFIO_HOME`, managed compiler state,
the detected tool release platform, the Styio client release target, and the
release-root source that `pafio install styio@latest` would reuse.
...
pafio publish --manifest-path path/to/pafio.toml --registry https://packages.example.test --registry-header 'X-Pafio-Write-Token: dev-token'
```
...
- `pafio doctor` publishes `doctor v1`
- the payload includes `ok`, `pafio_home`, `platform`, `release_root`, `checks`, and `tool_status`
...
`18` is also used by `pafio doctor` when a blocking local tool-management prerequisite is missing or invalid.
...
- `pafio install styio` and `pafio install styio@latest` default to stable latest and prefer a platform-hosted prebuilt compiler when a release root is configured
- prebuilt install resolves the release root from `--release-root`, `PAFIO_STYIO_RELEASE_ROOT`, `PAFIO_TOOL_RELEASE_ROOT`, or `PAFIO_HOME/config/tool-release-root`
...
- prebuilt install reads `tools/<release-target>/channel/<stable|nightly>/<platform>/version`, downloads `tools/<release-target>/releases/<version>/<platform>/styio`, verifies `styio.sha256`, and promotes the compiler to `PAFIO_HOME/tools/styio/current/`
- prebuilt install accepts legacy `tools/styio/...` channel and release paths only as a compatibility fallback
- `install-pafio.sh` writes a `styio` wrapper that delegates to the managed compiler under `PAFIO_HOME/tools/styio/current/bin/styio`; the compiler itself must answer `styio --version`
- `--source` forces source-build mode and `--prebuilt-only` fails instead of falling back to source-build
- source fallback fetches from `PAFIO_STYIO_SOURCE_ORIGIN` when set, otherwise from `https://github.com/SymPolicy/Styio-Preview.git`
- source fallback uses `PAFIO_STYIO_SOURCE_REF` when set, otherwise `main` for `latest`; explicit `styio@<ref>` maps to that source revision when prebuilt install is skipped or unavailable
- source fallback defaults to non-interactive fetch approval so fresh machines can bootstrap with one command
- both prebuilt and source installs validate the resulting compiler through the same compatibility matrix as `pafio tool install` before promoting it to `PAFIO_HOME/tools/styio/current/`
...
- `pafio build minimal` uses the selected project toolchain mode:
  ...
  - `build` resolves or fetches the official `styio` source tree from `https://github.com/SymPolicy/Styio-Preview.git`, maps `stable` and `nightly` to the same-named source branches, builds a local compiler under `PAFIO_HOME/toolchains/source/`, and then runs the compile-plan through that source-built compiler
- Source-build alignment requirements for `pafio build minimal`:
  - official source origin is `https://github.com/SymPolicy/Styio-Preview.git`
