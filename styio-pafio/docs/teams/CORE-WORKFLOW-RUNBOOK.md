...
3. Treat `pafio use`, `pafio set`, `pafio sync`, `pafio doctor`, `pafio project-graph --json`, `pafio tool status --json`, `pafio build minimal`, local import/export, offline package use, and `pafio-toolchain.lock` as owned workflow surface.
4. Keep the bootstrap path `curl ... install-pafio.sh | sh`, `pafio doctor`, `pafio install styio@latest`, and the managed `styio` shim runnable on a fresh VM before claiming installer closure.
...
./build-codex/bin/pafio_native_tests --gtest_filter='DoctorTests.*'
