...
Record the registry mode affected, the acceptance command that still fails, whether local or remote storage assumptions changed, and whether the next fix belongs in `styio-pafio` client code or `pafio` service code.
