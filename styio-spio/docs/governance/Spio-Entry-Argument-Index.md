...
- in `build` mode, the default official source origin is `https://github.com/eBioRing/styio-all-in-one.git`
...
- source fallback fetches source from `SPIO_STYIO_SOURCE_ORIGIN` when set, otherwise from `https://github.com/eBioRing/styio-all-in-one.git`
